...
for (var i=0; i<myData.length; i++) {
  ratings.push(myData[i].rt_score/100);
  titles.push(myData[i].title);
  release_dates.push(myData[i].release_date);
}
...
yScale=d3.scaleBand()
  .domain(titles)
//data used for "domain" needs to be a unique identifier
  .paddingInner(.1)
  .range([0,height]);
...
colors = d3.scaleLinear()
  .domain([0,0.8, d3.max(ratings)/100])
  .range (['#99f542','#eb7d34']);
...
myChart = d3.select('#barchart').append('svg')
   .attr('width', width + margin.left + margin.right)
   .attr('height', height + margin.top + margin.bottom)
   .append('g')
   .attr('transform',
      'translate(' + margin.left + ',' + margin.right + ')')
  .selectAll('rect').data(ratings)
    .enter().append('rect')
      .attr('fill', colors)
      ...
      .attr('width', function(d) {
        return xScale(d);
      //pass along data asscoiated xScale value
      })
      .attr('y', function(d,i) {
        return yScale(titles[i]);
    //reference that key in your yScale function to grab the value from the array.
      })
      .attr('x', 0);
...
//add label to chart but it didn't work  
// myLabel = d3.select('#barchart').append('svg')
//         .append('g')
//         .selectAll("text")        
//         .data(ratings).enter().append("text")
//         .attr("class","label")
//       .text(function (d) {
//         return d;})
//     .attr("x", function(d) {
...
//     return yScale(titles[i]);})
      ...
      //set x coordinate at starting point

xGuide = d3.select('#barchart svg')
        .append('g')
        .attr('transform','translate(250,'+ 480 +')')
        .attr("class", "x-axis")
      //set x coordinate at 100 position}
        .call(xAxisTicks)
        .selectAll('text')
      //set x coordinate at 100 position
